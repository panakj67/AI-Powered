...
export default function ProductivityPanel() {
  ...
  useEffect(() => { loadAll().catch(() => toast.error("Failed to load productivity modules")); }, []);
  useEffect(() => {
  ...
  const createReminder = async () => { await axios.post("/api/reminders", { title: "Follow up", datetime: new Date(Date.now() + 3600000), followUpSuggestion: "Do you want me to draft the submission email too?" }); loadAll(); };
  const createTask = async () => { await axios.post("/api/tasks", { title: "Auto task from chat", status: "todo", priority: "medium" }); loadAll(); };
  const draftEmail = async () => { await axios.post("/api/emails/draft", { to: "demo@example.com", subject: "Draft", body: "Generated draft" }); loadAll(); };
  const createEvent = async () => { await axios.post("/api/calendar", { title: "Weekly Planning", startTime: new Date(Date.now() + 86400000), endTime: new Date(Date.now() + 90000000) }, { headers: { "x-action-confirmed": "true" } }); loadAll(); };
  const saveMemory = async () => { await axios.post("/api/memory", { key: "tone", value: "concise and proactive", confidence: 0.8 }); loadAll(); };
  const createAgentRun = async () => { await axios.post("/api/agent-runs", { goal: "Plan my Goa trip under 20k" }); loadAll(); };
  const createWorkspace = async () => { await axios.post("/api/workspaces", { name: "Product Team" }); loadAll(); };
  ...
  const onDrop = async (e, status) => { const taskId = e.dataTransfer.getData("taskId"); await axios.patch(`/api/tasks/${taskId}`, { status }); loadAll(); };

  const secureAction = (endpoint) => setConfirmState({ open: true, endpoint, method: "post" });
  const doConfirm = async () => {
    await axios({ url: confirmState.endpoint, method: confirmState.method, headers: { "x-action-confirmed": "true", "x-action-pin": pin } });
  ...
  return (
    <div className="mt-4 p-3 border rounded-xl space-y-3">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
        <div>Reminders: {reminders.length}</div><div>Events: {events.length}</div>
        <div>Agent Runs: {agentRuns.length}</div><div>Emails: {emails.length}</div>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
        {columns.map((col) => (
          <div key={col} onDragOver={(e) => e.preventDefault()} onDrop={(e) => onDrop(e, col)} className="border rounded p-2 min-h-16">
            ...
            {tasks.filter((t) => t.status === col).map((t) => <div key={t._id} draggable onDragStart={(e) => onDragStart(e, t._id)} className="mt-1 p-1 bg-gray-100 rounded text-xs text-black">{t.title}</div>)}
          </div>
        ))}
      </div>
      {emails.map((e) => <div key={e._id} className="text-xs">{e.subject} ({e.status}) <button className="underline" onClick={() => secureAction(`/api/emails/${e._id}/approve`)}>Approve</button> <button className="underline" onClick={() => secureAction(`/api/emails/${e._id}/send`)}>Send</button></div>)}
